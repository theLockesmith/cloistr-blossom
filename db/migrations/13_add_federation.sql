...
CREATE TABLE IF NOT EXISTS federated_blobs (
    ...
    discovered_at   INTEGER NOT NULL,              -- Unix timestamp when first seen
    mirrored_at     INTEGER,                       -- Unix timestamp when mirrored locally (NULL if not mirrored)
    last_seen_at    INTEGER NOT NULL               -- Unix timestamp of most recent kind 1063 event
);
...
CREATE TABLE IF NOT EXISTS known_servers (
    ...
    last_check  INTEGER                            -- Last health check timestamp
);
...
CREATE TABLE IF NOT EXISTS federation_events (
    ...
    created_at  INTEGER NOT NULL,
    published_at INTEGER,                          -- When event was published (NULL if pending/failed)
